...
public abstract class AbstractCommand extends net.minecraft.command.CommandBase {
    ...
    public ITextComponent checkRequirements(ICommandSender sender, String[] params) {
    	...
    	AppliedPatches clientInfo = isSenderOfEntityType(sender, EntityPlayerMP.class) ? 
    	PatchManager.instance().getAppliedPatchesForPlayer(getSenderAsEntity(sender, EntityPlayerMP.class)) : null;
    	
    	CommandRequirement[] requierements = this.getRequirements();
    	
    	if (clientInfo == null)
    		clientInfo = AppliedPatches.PATCHES_CAPABILITY.getDefaultInstance();
