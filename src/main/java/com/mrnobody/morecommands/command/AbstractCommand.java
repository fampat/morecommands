...
import com.mrnobody.morecommands.command.CommandSender.EntityCommandSenderWrapper;
...
public abstract class AbstractCommand extends net.minecraft.command.CommandBase {
	...
	public boolean canCommandSenderUseCommand(ICommandSender p_71519_1_) {
		...
		else return p_71519_1_.canCommandSenderUseCommand(this.getRequiredPermissionLevel(), this.getCommandName());
	}
	
	/**
	 * @return whether the command sender can use this command including execution arguments
	 */
	public boolean canCommandSenderUseCommand(ICommandSender sender, String[] params) {
	 ...
	 */
    public abstract int getDefaultPermissionLevel(String[] params);
    ...
    /**
     * Makes a chat component with red color
     * 
     * @param msg the message
     * @return the chat component
     */
    protected final IChatComponent makeChatMsg(String msg) {
     ...
     * @return the rejoined parameters
     */
    public static final String rejoinParams(Iterable<String> params) {
    	return spaceJoiner.join(params);
    }
     ...
     */
    public static NBTBase getNBTFromParam(String param) {
    	if (isNBTParam(param)) {
    		NBTBase nbt = null;
    		
    		try {return JsonToNBT.func_150315_a(param);}
     ...
     */
    public static Coordinate getCoordFromParams(ICommandSender sender, String[] params, int index) throws NumberFormatException {
		...
		int x = relative ? sender.getPlayerCoordinates().posX + Integer.parseInt(params[index].substring(1)) : Integer.parseInt(params[index]);
		
		relative = params[index + 1].startsWith("~"); if (relative && params[index + 1].length() <= 1) params[index + 1] = "~0";
		int y = relative ? sender.getPlayerCoordinates().posY + Integer.parseInt(params[index + 1].substring(1)) : Integer.parseInt(params[index + 1]);
		
		relative = params[index + 2].startsWith("~"); if (relative && params[index + 2].length() <= 1) params[index + 2] = "~0";
		int z = relative ? sender.getPlayerCoordinates().posZ + Integer.parseInt(params[index + 2].substring(1)) : Integer.parseInt(params[index + 2]);
    ...
    public static Item getItem(String param) {
    	if (Item.itemRegistry.containsKey(param))
    		return (Item) Item.itemRegistry.getObject(param);
    	else {
    		try {return Item.getItemById(Integer.parseInt(param));}
    ...
    public static Potion getPotion(String param) {
    	if (param.startsWith("potion.")) param = param.substring("potion.".length());
    	
    	for (Potion p : Potion.potionTypes) {
    		if (p == null) continue;
    		String name = p.getName().startsWith("potion.") ? p.getName().substring("potion.".length()) : p.getName();
    		if (param.equals(name) || param.equals(String.valueOf(p.getId()))) return p;
    	}
    ...
    public static Enchantment getEnchantment(String param) {
    	if (param.startsWith("enchantment.")) param = param.substring("enchantment.".length());
    	
    	for (Enchantment e : Enchantment.enchantmentsList) {
    		if (e == null) continue;
    		String name = e.getName().startsWith("enchantment.") ? e.getName().substring("enchantment.".length()) : e.getName();
    		if (param.equals(name) || param.equals(String.valueOf(e.effectId))) return e;
    	}
     ...
     */
    public static EntityPlayerMP getPlayer(String param) {
    	return MinecraftServer.getServer().getConfigurationManager().func_152612_a(param);
    }
    ...
    public static <T extends Entity> boolean isSenderOfEntityType(ICommandSender sender, Class<T> entityType) {
    	return entityType.isInstance(sender instanceof EntityCommandSenderWrapper ? ((EntityCommandSenderWrapper) sender).getEntity() : sender);
    }
    ...
    public static <T extends Entity> T getSenderAsEntity(ICommandSender sender, Class<T> entityType) {
    	...
    	else return entityType.cast(sender instanceof EntityCommandSenderWrapper ? ((EntityCommandSenderWrapper) sender).getEntity() : sender);
    }
    ...
    public static ServerPlayerSettings getPlayerSettings(EntityPlayerMP player) {
    	...
    	if (settings == null) {
    		settings = new ServerPlayerSettings(player);
    ...
    /**
     * Gets the {@link ClientPlayerSettings} corresponding to the given {@link EntityClientPlayerMP}<br>
    ...
    public static ClientPlayerSettings getPlayerSettings(EntityClientPlayerMP player) {
    	...
    	if (settings == null) {
    		settings = ClientPlayerSettings.getInstance(player);
    		player.registerExtendedProperties(PlayerSettings.MORECOMMANDS_IDENTIFIER, settings);
    	}
    	return settings;
    }
