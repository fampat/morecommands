...
public class CommandChatstyle extends MultipleCommands implements ServerCommandProperties, EventListener<ServerChatEvent> {
	...
	public void onEvent(ServerChatEvent event) {
		if (!event.component.getKey().equals("chat.type.text") || event.component.getFormatArgs().length != 2 ||
			!(event.component.getFormatArgs()[0] instanceof IChatComponent) ||
			!(event.component.getFormatArgs()[1] instanceof IChatComponent)) return;
		
		ServerPlayerSettings settings = getPlayerSettings(event.player);
		
		if (settings.nameStyle != null) 
			((IChatComponent) event.component.getFormatArgs()[0]).getChatStyle().setParentStyle(settings.nameStyle);
		
		if (settings.textStyle != null) 
			((IChatComponent) event.component.getFormatArgs()[1]).getChatStyle().setParentStyle(settings.textStyle);
	}
