...
public class CommandSpawnportal extends StandardCommand implements ServerCommandProperties {
	private final Method createEnderPortal = ReflectionHelper.getMethod(ObfuscatedMethod.EntityDragon_createEnderPortal);
	...
	public void execute(CommandSender sender, String[] params) throws CommandException {
		...
		if (params.length > 0) {
			if (params[0].equalsIgnoreCase("end")) {
				Coordinate coord = new Coordinate(entity);
				...
				try {this.createEnderPortal.invoke(new EntityDragon(entity.worldObj), x, z);}
				catch (Throwable t) {
					t.printStackTrace();
