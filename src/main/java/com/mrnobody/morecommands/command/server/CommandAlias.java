		...
		if (event.command instanceof DummyCommand && !((DummyCommand) event.command).isClient()) {
			...
			if (isSenderOfEntityType(event.sender, EntityPlayerMP.class)) {
				...
				if (command == null && MoreCommandsConfig.enableGlobalAliases)
					command = GlobalSettings.getInstance().aliases.get(ImmutablePair.of(event.sender.getEntityWorld().getSaveHandler().getWorldDirectoryName(), event.sender.getEntityWorld().provider.getDimensionName())).get(event.command.getCommandName());
