...
public class CommandAchievement extends StandardCommand implements ServerCommandProperties {
	...
	static {
		...
		Map<String, StatBase> stats = (Map<String, StatBase>) ReflectionHelper.get(ObfuscatedField.StatList_ID_TO_STAT_MAP, null);
		if (stats != null) {
    ...
    public String execute(CommandSender sender, String[] params) throws CommandException {
    	...
    	if (params.length > 0) {
    		if(params[0].equals("list")) {
    			...
    			sender.sendLangfileMessage("command.achievement.more", TextFormatting.RED);
    		}
    			...
    			if (params[1].equals("*")) {
    				...
    				List<Achievement> lock = Lists.newArrayList(Iterators.filter(AchievementList.ACHIEVEMENTS.iterator(), new Predicate<Achievement>() {
