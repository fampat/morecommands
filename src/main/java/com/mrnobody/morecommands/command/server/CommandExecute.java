...
public class CommandExecute extends MultipleCommands implements ServerCommandProperties {
		...
		if (!commandName.equals("execute")) {
			...
			if (action.equals("confirm") || action.equals("deny")) {
				...
				if (request == null) throw new CommandException("command.execute.invalidRequest", sender, params.length > 0 ? Integer.parseInt(params[0]) : 0);
					...
					else new CommandSender(request.getLeft()).sendLangfileMessage("command.execute.denial", EnumChatFormatting.RED, player.getCommandSenderName(), request.getRight().command);
				}
			}
			else {
	...
	private boolean executeCommand(Entity entity, ICommandSender sender, Coordinate relCoord, String command, boolean force) {
		if (!force && MoreCommandsConfig.useExecRequests && entity instanceof EntityPlayerMP && playerHasRequestLevel((EntityPlayerMP) entity)) {
			...
			ChatComponentText confirmText = new ChatComponentText(LanguageManager.translate(
					MoreCommands.INSTANCE.getCurrentLang((EntityPlayerMP) entity), "command.execute.confirmText", sender.getCommandSenderName(), command, request));
		...
		else MinecraftServer.getServer().getCommandManager().executeCommand(
			new CommandSender.EntityCommandSenderWrapper(entity, sender, relCoord), command);
	...
	private static boolean playerHasRequestLevel(EntityPlayerMP player) {
		if (MoreCommandsConfig.minExecRequestLevel == 0) return true;
			...
			if (MinecraftServer.getServer().getConfigurationManager().func_152596_g(player.getGameProfile())) {
				UserListOpsEntry entry = (UserListOpsEntry) MinecraftServer.getServer().getConfigurationManager().func_152603_m().func_152683_b(player.getGameProfile());
				return entry != null ? entry.func_152644_a() >= MoreCommandsConfig.minExecRequestLevel : MinecraftServer.getServer().getOpPermissionLevel() >= MoreCommandsConfig.minExecRequestLevel;
			}
			else return false;
		}
	}
	...
	private static int addRequest(Entity entity, ICommandSender sender, Coordinate relCoord, String command) {
		CommandExecutionRequests cetc = MoreCommands.getEntityProperties(CommandExecutionRequests.class, REQUESTS_KEY, entity);
	...
	private static final class ExecutionRequest {
		...
		public final Coordinate relCoord;
		public final String command;
		public final long creationTime = System.currentTimeMillis();
		
		public ExecutionRequest(Coordinate relCoord, String command, int requestID) {
	...
	private static class CommandExecutionRequests implements IExtendedEntityProperties {
		...
		private void cleanup() {
			...
			CommandSender notify = this.entity instanceof ICommandSender ? new CommandSender((ICommandSender) this.entity) : null;
			...
			while (itr.hasNext()) {
				...
				if (System.currentTimeMillis() - er.getValue().creationTime > MoreCommandsConfig.execRequestTimeout * 1000) {
					...
					if (s != null && this.entity != null) 
						new CommandSender(s).sendLangfileMessage("command.execute.senderRequestTimeout", EnumChatFormatting.RED, this.entity.getCommandSenderName(), er.getValue().command);
				}
			}
		}
		...
		public ListMultimap<String, ExecutionRequest> getRequests() {
			...
			for (Map.Entry<WeakReference<ICommandSender>, Collection<ExecutionRequest>> entry : this.requests.asMap().entrySet()) {
				...
				if (sender != null) list.putAll(sender.getCommandSenderName(), entry.getValue());
			}
		...
		public int addRequest(ICommandSender sender, Coordinate relCoord, String command) {
			cleanup();
