...
public class CommandMelt extends StandardCommand implements ServerCommandProperties {
	...
	public String execute(CommandSender sender, String[] params)throws CommandException {
		...
		if (!all) {
			if (player.getItemStackFromSlot(EntityEquipmentSlot.MAINHAND) == ItemStack.EMPTY) return null;
			result = FurnaceRecipes.instance().getSmeltingResult(player.getItemStackFromSlot(EntityEquipmentSlot.MAINHAND));
			if (result != ItemStack.EMPTY) player.setItemStackToSlot(EntityEquipmentSlot.MAINHAND, new ItemStack(result.getItem(),
													player.getItemStackFromSlot(EntityEquipmentSlot.MAINHAND).getCount(), result.getItemDamage()));
		}
		else {
			for (int i = 0; i < player.inventory.getSizeInventory(); i++) {
				...
				if (result != ItemStack.EMPTY) player.inventory.setInventorySlotContents(i, new ItemStack(result.getItem(),
														player.inventory.getStackInSlot(i).getCount(), result.getItemDamage()));
				if (result != ItemStack.EMPTY) smelt++;
			}
		}
