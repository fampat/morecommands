	...
	public void execute(CommandSender sender, String[] params) throws CommandException {
		if (params.length > 2 && params[0].equalsIgnoreCase("set")) {
			...
			if (command instanceof ServerCommand<?>) {
				...
				sender.sendLangfileMessage("command.permission.permissionSet", command.getName(), ((ServerCommand<?>) command).getRequiredPermissionLevel());
			}
			...
			if (command instanceof CommandBase)
				sender.sendLangfileMessage("command.permission.getPermission", command.getName(), ((CommandBase) command).getRequiredPermissionLevel());
			else throw new CommandException("command.permission.invalidCommand", sender);
		}
			...
			if (command instanceof ServerCommand<?>) {
				GlobalSettings.permissionMapping.remove(command.getName());
				((ServerCommand<?>) command).refreshPermissionLevel();
				sender.sendLangfileMessage("command.permission.reseted", command.getName(), ((ServerCommand<?>) command).getRequiredPermissionLevel());
			}
			else throw new CommandException("command.permission.invalidCommand", sender);
		}
