...
public class CommandPermission extends StandardCommand implements ServerCommandProperties {
	...
	public String execute(CommandSender sender, String[] params) throws CommandException {
		...
		if ((params.length > 3 && params[0].equalsIgnoreCase("setaction")) || (params.length > 2 && params[0].equalsIgnoreCase("setbase"))) {
			...
			level = Math.min(sender.getServer().getOpPermissionLevel(), level);
			...
			if (command instanceof ServerCommand<?>) {
				...
				if (action)
					sender.sendLangfileMessage("command.permission.actionPermissionSet", command.getName(), params[2],
							((ServerCommand<?>) command).getRequiredPermissionLevel(new String[] {params[2]}));
				else
					sender.sendLangfileMessage("command.permission.basePermissionSet", command.getName(),
			...
			ICommand command = sender.getServer().getCommandManager().getCommands().get(name);
			
			if (action && command instanceof CommandBase<?>)
				sender.sendLangfileMessage("command.permission.actionPermissionGet", command.getName(), params[2],
				...
				sender.sendLangfileMessage("command.permission.basePermissionGet", command.getName(), 
			...
			if (command instanceof ServerCommand<?>) {
				...
				if (type.equalsIgnoreCase("action"))
					sender.sendLangfileMessage("command.permission.actionPermissionReset", command.getName(), params[2],
												((ServerCommand<?>) command).getRequiredPermissionLevel(new String[] {params[2]}));
				else if (type.equalsIgnoreCase("base"))
					sender.sendLangfileMessage("command.permission.basePermissionReset", command.getName(),
												((ServerCommand<?>) command).getRequiredPermissionLevel());	
				else 
					sender.sendLangfileMessage("command.permission.allReset", command.getName());
			}
			else throw new CommandException("command.permission.invalidCommand", sender);
		}
