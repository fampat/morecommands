	...
	public String execute(CommandSender sender, String[] params)throws CommandException {
		...
		if (params.length > 0 && params[0].equalsIgnoreCase("all")) {
			for (int i = 0; i < player.inventory.getSizeInventory(); i++) {
				if (player.inventory.getStackInSlot(i) != ItemStack.field_190927_a) 
					player.inventory.getStackInSlot(i).func_190920_e(player.inventory.getStackInSlot(i).getMaxStackSize());
			}
		}
		else {
			if (player.getItemStackFromSlot(EntityEquipmentSlot.MAINHAND) != ItemStack.field_190927_a) 
				player.getItemStackFromSlot(EntityEquipmentSlot.MAINHAND).func_190920_e(player.getItemStackFromSlot(EntityEquipmentSlot.MAINHAND).getMaxStackSize());
			else
