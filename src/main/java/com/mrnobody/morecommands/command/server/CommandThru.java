...
public class CommandThru extends StandardCommand implements ServerCommandProperties {
	...
	public String execute(CommandSender sender, String[] params) throws CommandException {
		...
		Vec3 lookVec = entity.getLook(1F);
		Vec3 traceVec = posVec.addVector(lookVec.xCoord * distance, 0, lookVec.zCoord * distance);
		
		MovingObjectPosition trace = entity.worldObj.rayTraceBlocks(posVec, traceVec, false);
    	...
    	for (int step = 1; step < distance; step++) {
    		if (WorldUtils.canStand(entity.worldObj, new Coordinate(trace.blockX + stepX * step, trace.blockY, trace.blockZ + stepZ * step))) {
    			EntityUtils.setPosition(entity, new Coordinate(trace.blockX + stepX * step, trace.blockY, trace.blockZ + stepZ * step));
