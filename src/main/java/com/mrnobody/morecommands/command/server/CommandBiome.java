...
public class CommandBiome extends StandardCommand implements ServerCommandProperties {
	...
	public String execute(CommandSender sender, String[] params) throws CommandException {
		...
		if (params.length > 0) {
			if (params[0].equalsIgnoreCase("info")) sender.sendLangfileMessage("command.biome.info", biome.getBiomeName(), BiomeGenBase.getIdForBiome(biome));
			else if (params[0].equalsIgnoreCase("list")) {
    			int page = 0;
    			ResourceLocation[] biomes = BiomeGenBase.biomeRegistry.getKeys().toArray(new ResourceLocation[BiomeGenBase.biomeRegistry.getKeys().size()]);
    			...
    			for (int i = page * PAGE_MAX; i < stop && i < biomes.length; i++)
    				sender.sendStringMessage(" - '" + biomes[i] + "' (ID " + BiomeGenBase.getIdForBiome(BiomeGenBase.biomeRegistry.getObject(biomes[i])) + ")");
