...
public class CommandChest extends StandardCommand implements ServerCommandProperties {
	...
	public String execute(CommandSender sender, String[] params)throws CommandException {
		...
		Block block = WorldUtils.getBlock(player.world, coord);
		...
		if (params[0].equalsIgnoreCase("drop")) {
            y1 += 1; y2 += 1;
            WorldUtils.setBlock(player.world, new BlockPos(x1, y1, z1), Blocks.CHEST);
            WorldUtils.setBlock(player.world, new BlockPos(x2, y2, z2), Blocks.CHEST);
		}
			...
			if (WorldUtils.getBlock(player.world, coord) == Blocks.CHEST) {
				if (WorldUtils.getBlock(player.world, x2, y2, z2) == Blocks.CHEST);
				else if (WorldUtils.getBlock(player.world, x1 - 1, y1, z1) == Blocks.CHEST) x2 = x1 - 1;
				else if (WorldUtils.getBlock(player.world, x1, y1, z1 + 1) == Blocks.CHEST) {x2 = x1; z2 = z1 + 1;}
				else if (WorldUtils.getBlock(player.world, x1, y1, z1 - 1) == Blocks.CHEST) {x2 = x1; z2 = z1 - 1;}
				else y2 = -1;
			}
		...
		if (y2 > -1) chest = new InventoryLargeChest("Large chest", (TileEntityChest) player.world.getTileEntity(new BlockPos(x1, y1, z1)), 
							(TileEntityChest) player.world.getTileEntity(new BlockPos(x2, y2, z2)));
		else chest = (TileEntityChest) player.world.getTileEntity(new BlockPos(x1, y1, z1));
	...
	private void transferInventory(IInventory from, IInventory to) {
		...
		try {
			for (int i = 0; i < from.getSizeInventory(); i++) {
				if (to == null) {
					from.setInventorySlotContents(i, ItemStack.EMPTY);
					continue;
				}
				
				try {
					while (to.getStackInSlot(count) != ItemStack.EMPTY) count++;
