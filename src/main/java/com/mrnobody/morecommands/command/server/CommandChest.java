...
public class CommandChest extends StandardCommand implements ServerCommandProperties {
	...
	public String execute(CommandSender sender, String[] params)throws CommandException {
		...
		if (y2 > -1) chest = new InventoryLargeChest("Large chest", (TileEntityChest) player.worldObj.getTileEntity(new BlockPos(x1, y1, z1)), 
							(TileEntityChest) player.worldObj.getTileEntity(new BlockPos(x2, y2, z2)));
		else chest = (TileEntityChest) player.worldObj.getTileEntity(new BlockPos(x1, y1, z1));
	...
	private void transferInventory(IInventory from, IInventory to) {
		...
		try {
			for (int i = 0; i < from.getSizeInventory(); i++) {
				if (to == null) {
					from.setInventorySlotContents(i, ItemStack.field_190927_a);
					continue;
				}
				
				try {
					while (to.getStackInSlot(count) != ItemStack.field_190927_a) count++;
