...
public class CommandChest extends StandardCommand implements ServerCommandProperties {
	...
	public String execute(CommandSender sender, String[] params)throws CommandException {
		...
		Coordinate coord = EntityUtils.traceBlock(player, 128D);
		...
		if (params[0].equalsIgnoreCase("drop")) {
            y1 += 1; y2 += 1;
            WorldUtils.setBlock(player.worldObj, x1, y1, z1, Blocks.chest);
            WorldUtils.setBlock(player.worldObj, x2, y2, z2, Blocks.chest);
		}
		...
		if (y2 > -1) chest = new InventoryLargeChest("Large chest", (TileEntityChest) player.worldObj.getTileEntity(x1, y1, z1), (TileEntityChest) player.worldObj.getTileEntity(x2, y2, z2));
		else chest = (TileEntityChest) player.worldObj.getTileEntity(x1, y1, z1);
