	...
	public String execute(CommandSender sender, String[] params) throws CommandException {
		...
		if (params.length > 0) {
    		if(params[0].equals("list")) {
    			int page = 0;
    			ResourceLocation[] potions = Potion.getPotionLocations().toArray(new ResourceLocation[Potion.getPotionLocations().size()]);
    			...
    			for (int i = page * PAGE_MAX; i < stop && i < potions.length; i++)
    				sender.sendStringMessage(" - '" + potions[i] + "' (ID " + Potion.getPotionFromResourceLocation(potions[i].toString()).getId() + ")");
