    		...
    		if(params[0].equals("list")) {
    			...
    			if (params.length > 2) {
    				...
    				for (Potion p : Potion.potionTypes) {
    					if (p != null) {
    						if (params[1].toLowerCase().equals(p.getName().substring(7).toLowerCase()) || String.valueOf(p.getId()).equals(params[1])) {
    							try {player.addPotionEffect(p.getId(), Integer.parseInt(params[2]), params.length > 3 ? Integer.parseInt(params[3]) : 0); broken = true; sender.sendLangfileMessageToPlayer("command.effect.addSuccess", new Object[0]);}
