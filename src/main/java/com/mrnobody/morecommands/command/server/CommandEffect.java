	...
	public void execute(CommandSender sender, String[] params) throws CommandException {
		...
		if (params.length > 0) {
    		if(params[0].equals("list")) {
    			int page = 0;
    			ResourceLocation[] potions = Potion.potionRegistry.getKeys().toArray(new ResourceLocation[Potion.potionRegistry.getKeys().size()]);
    			...
    			for (int i = page * PAGE_MAX; i < stop && i < potions.length; i++)
    				sender.sendStringMessage(" - '" +  potions[i] + "' (ID " + Potion.getIdFromPotion(Potion.potionRegistry.getObject(potions[i])) + ")");
				...
				if (potion != null) entity.removePotionEffect(potion);
				...
				Potion potion = getPotion(params[1]);
				if (potion != null) entity.addPotionEffect(potion, duration, strength, invisible);
				else throw new CommandException("command.effect.notFound", sender);
