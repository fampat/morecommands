    		...
    		if(params[0].equals("list")) {
    			...
    			if (params.length > 3) {
    				...
    				for (Potion p : Potion.potionTypes) {
    					if (p != null) {
    						if (params[1].equalsIgnoreCase(p.getName().substring(7)) || String.valueOf(p.getId()).equals(params[1])) {
    							try {player.addPotionEffect(p.getId(), Integer.parseInt(params[2]), Integer.parseInt(params[3])); broken = true; sender.sendLangfileMessage("command.effect.addSuccess", new Object[0]);}
