...
public class CommandFirework extends StandardCommand implements ServerCommandProperties {
	...
	public String execute(CommandSender sender, String[] params) throws CommandException {
		EntityLivingBase entity = params.length > 2 ? null : 
			isSenderOfEntityType(sender.getMinecraftISender(), EntityLivingBase.class) ? 
			getSenderAsEntity(sender.getMinecraftISender(), EntityLivingBase.class) : null;
		Coordinate spawn = null;
		
		if (params.length > 2)
			spawn = getCoordFromParams(sender.getMinecraftISender(), params, 0);
		else
			spawn = entity == null ? sender.getPosition() : EntityUtils.traceBlock(entity, 128D);
