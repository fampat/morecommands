	...
	public void execute(CommandSender sender, String[] params) throws CommandException {
		...
		if (params.length > 2) {
			try {
				...
				if (params.length > 3)
					yaw = params[3].equals("~") ? yaw : MathHelper.wrapDegrees(Float.parseFloat(params[3]));
				
				if (params.length > 4)
					pitch = params[4].equals("~") ? pitch : MathHelper.wrapDegrees(Float.parseFloat(params[4]));
				
				if (yaw > 90F || yaw < -90F) yaw = MathHelper.wrapDegrees(yaw + 180F);
				if (pitch > 90F || pitch < -90F) pitch = MathHelper.wrapDegrees(pitch + 180F);
				
				if (settings != null) 
					settings.lastTeleport = settings.lastPos = entity.getPosition();
				
				if (entity.getMinecraftEntity() instanceof EntityPlayerMP)
					((EntityPlayerMP) entity.getMinecraftEntity()).connection.setPlayerLocation(coord.getX(), coord.getY(), coord.getZ(), yaw, pitch);
				else
