...
public enum CommandRequirement {
	...
	PATCH_NETHANDLERPLAYSERVER("command.generic.netServerPlayHandlerNotPatched") {
		@Override public boolean isSatisfied(ICommandSender sender) {
			...
			return player == null ? false : player.playerNetServerHandler instanceof PatchNetHandlerPlayServer.NetHandlerPlayServer;
		}
	},
	/**
	 * Checks whether {@link Minecraft#thePlayer} is of type {@link com.mrnobody.morecommands.patch.PatchEntityClientPlayerMP.EntityClientPlayerMP}
	 */
	PATCH_ENTITYCLIENTPLAYERMP("command.generic.clientPlayerNotPatched") {
		@Override public boolean isSatisfied(ICommandSender sender) {
			if (MoreCommands.isClientSide()) {
				PatchEntityClientPlayerMP.EntityClientPlayerMP player = getAsEntityOrNull(Minecraft.getMinecraft().thePlayer, PatchEntityClientPlayerMP.EntityClientPlayerMP.class);
				return player != null;
			}
			else {
				EntityPlayerMP player = getAsEntityOrNull(sender, EntityPlayerMP.class);
				return player == null ? false : PatchManager.instance().getAppliedPatchesForPlayer(player).wasPatchSuccessfullyApplied(PatchList.PATCH_ENTITYCLIENTPLAYERMP);
			}
		}
	},
	/**
	...
	PATCH_SERVERCONFIGMANAGER("command.generic.serverConfigManagerNotPatched") {
		@Override public boolean isSatisfied(ICommandSender sender) {
			...
			return mcServer == null ? false : mcServer.getConfigurationManager().getClass().getName().startsWith(PatchEntityPlayerMP.class.getName());
		}
	},
	/**
