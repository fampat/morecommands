...
public class CommandAlias extends MultipleCommands implements ClientCommandProperties, EventListener<CommandEvent> {
	...
	public void onEvent(CommandEvent event) {
		if (event.command instanceof DummyCommand && ((DummyCommand) event.command).isClient()) {
			...
			if (MoreCommandsConfig.enablePlayerAliases && isSenderOfEntityType(event.sender, EntityClientPlayerMP.class))
				command = getPlayerSettings(getSenderAsEntity(event.sender, EntityClientPlayerMP.class)).aliases.get(event.command.getCommandName());
	...
	public String execute(String commandName, CommandSender sender, String[] params) throws CommandException {
		...
		if (!isSenderOfEntityType(sender.getMinecraftISender(), EntityClientPlayerMP.class)) 
			...
			settings = getPlayerSettings(getSenderAsEntity(sender.getMinecraftISender(), EntityClientPlayerMP.class));
