    ...
    public final void execute(ICommandSender sender, String[] params) {
    	if (MoreCommands.isModEnabled() && this.isEnabled(sender)) {
    		...
    		if (!MoreCommands.isModEnabled())
    			sender.addChatMessage(new ChatComponentText(LanguageManager.translate(MoreCommands.getMoreCommands().getCurrentLang(sender), "command.generic.notEnabled", new Object[0])));
    		else if (!(sender instanceof EntityPlayerMP))
    			sender.addChatMessage(new ChatComponentText(LanguageManager.translate(MoreCommands.getMoreCommands().getCurrentLang(sender), "command.generic.notServer", new Object[0])));
    	}
    }
    	...
    	for (Requirement requierement : requierements) {
    		...
    		if (requierement == Requirement.PATCH_RENDERGLOBAL) {
    			if (!clientInfo.renderGlobalPatched()) {
    				sendChatMsg(sender, LanguageManager.translate(lang, "command.generic.renderGlobalNotPatched"));
    	    		return false;
    			}
    		}
