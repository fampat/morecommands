...
public class Player extends com.mrnobody.morecommands.wrapper.EntityLivingBase {
	...
	public void sendChatMessage(String message) {
		this.player.addChatMessage(new TextComponentString(message));
	}
	...
	public String getGameType() {
		return this.player.interactionManager.getGameType().getName();
	}
	...
	public void removeAchievement(Achievement ach) {
        this.player.getStatFile().unlockAchievement(this.player, ach, 0);
        Iterator iterator = this.player.getWorldScoreboard().getObjectivesFromCriteria(ach.getCriteria()).iterator();

        while (iterator.hasNext())
        {
            ScoreObjective scoreobjective = (ScoreObjective) iterator.next();
            this.player.getWorldScoreboard().getOrCreateScore(this.player.getName(), scoreobjective).setScorePoints(0);
        }

        if (this.player.getStatFile().hasUnsentAchievement())
        {
