...
public class PatchChatGui implements PatchManager.ForgeEventBasedPatch {
	...
	public <T extends Event> boolean needsToBeApplied(T e) {
		return ((GuiOpenEvent) e).getGui() instanceof net.minecraft.client.gui.GuiChat;
	}
	...
	private static void processChat(Minecraft mc, String chat, boolean addToChat) {
		...
		if (MoreCommandsConfig.enablePlayerVars) {
			ClientPlayerSettings settings = mc.player.getCapability(PlayerSettings.SETTINGS_CAP_CLIENT, null);
		...
		if (chat.trim().startsWith("/") && ClientCommandHandler.instance.executeCommand(mc.player, chat) != 0) return;
		else mc.player.sendChatMessage(chat);
	}
}
