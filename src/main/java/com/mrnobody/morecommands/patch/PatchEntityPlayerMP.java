...
public class PatchEntityPlayerMP implements PatchManager.StateEventBasedPatch {
	...
	private static void transferPlayerToDimension(ServerConfigurationManager scm, net.minecraft.entity.player.EntityPlayerMP player, int dimension, Teleporter teleporter) {
        ...
        player.playerNetServerHandler.sendPacket(new S07PacketRespawn(player.dimension, worldserver1.getDifficulty(), worldserver1.getWorldInfo().getTerrainType(), player.theItemInWorldManager.getGameType())); // Forge: Use new dimensions information
        ...
        net.minecraftforge.fml.common.FMLCommonHandler.instance().firePlayerChangedDimensionEvent(player, j, dimension);
    }
