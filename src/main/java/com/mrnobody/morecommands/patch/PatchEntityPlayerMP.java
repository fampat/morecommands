...
public class PatchEntityPlayerMP implements PatchManager.StateEventBasedPatch {
	...
	private static net.minecraft.entity.player.EntityPlayerMP createPlayerForUser(PlayerList playerList, GameProfile profile) {
		...
		for (net.minecraft.entity.player.EntityPlayerMP player : playerList.getPlayerList())
	...
	private static net.minecraft.entity.player.EntityPlayerMP recreatePlayerEntity(PlayerList playerList, GameType myGameType, 
		...
		player.getServerForPlayer().getPlayerChunkManager().removePlayer(player);
        
		playerList.getPlayerList().remove(player);
        ...
        while (!worldserver.getCubes(newPlayer, newPlayer.getEntityBoundingBox()).isEmpty() && newPlayer.posY < 256.0D)
        	newPlayer.setPosition(newPlayer.posX, newPlayer.posY + 1.0D, newPlayer.posZ);
        
        newPlayer.playerNetServerHandler.sendPacket(new SPacketRespawn(newPlayer.dimension, newPlayer.worldObj.getDifficulty(), newPlayer.worldObj.getWorldInfo().getTerrainType(), newPlayer.interactionManager.getGameType()));
        ...
        newPlayer.playerNetServerHandler.sendPacket(new SPacketSpawnPosition(blockpos2));
        ...
        worldserver.getPlayerChunkManager().addPlayer(newPlayer);
        worldserver.spawnEntityInWorld(newPlayer);
        
        playerList.getPlayerList().add(newPlayer);
	...
	private static void transferPlayerToDimension(PlayerList playerList, net.minecraft.entity.player.EntityPlayerMP player, int dimension, Teleporter teleporter) {
        ...
        player.playerNetServerHandler.sendPacket(new SPacketSpawnPosition(new BlockPos(worldserver1.getWorldInfo().getSpawnX(), worldserver1.getWorldInfo().getSpawnY(), worldserver1.getWorldInfo().getSpawnZ())));
