...
public class PatchNetHandlerPlayServer implements PatchManager.ForgeEventBasedPatch {
	...
	public <T extends Event> boolean applyForgeEventPatch(T e) {
		...
		if (player.playerNetServerHandler.playerEntity == event.getEntity() && !(player.playerNetServerHandler instanceof NetHandlerPlayServer)) {
			net.minecraft.network.NetHandlerPlayServer handler = player.playerNetServerHandler;
			player.playerNetServerHandler = new NetHandlerPlayServer(player.getServer(), handler.netManager, handler.playerEntity);
	...
	public static class NetHandlerPlayServer extends net.minecraft.network.NetHandlerPlayServer {
	    ...
	    NetHandlerPlayServer(MinecraftServer par1, NetworkManager par2, net.minecraft.entity.player.EntityPlayerMP par3) {
