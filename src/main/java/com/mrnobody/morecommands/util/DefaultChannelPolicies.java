...
public final class DefaultChannelPolicies {
	...
	private static abstract class AbstractChannelPolicy implements ChannelPolicy {
		...
		public void kickMessage(EntityPlayerMP player, ChatChannel kickedFrom, ChatChannel newChannel, boolean isPolicyChange) {
			...
			if (isPolicyChange) {
				if (newChannel == null) sender.sendLangfileMessage("command.chatchannel.policychange.nullChannel", EnumChatFormatting.RED, kickedFrom.getName());
				else sender.sendLangfileMessage("command.chatchannel.policychange.newChannel", EnumChatFormatting.RED, kickedFrom.getName(), newChannel.getName());
			}
			else {
				if (newChannel == null) sender.sendLangfileMessage("command.chatchannel.destroyed.nullChannel", EnumChatFormatting.RED, kickedFrom.getName());
				else sender.sendLangfileMessage("command.chatchannel.destroyed.newChannel", EnumChatFormatting.RED, kickedFrom.getName(), newChannel.getName());
			}
		}
	}
	...
	public static final class WhitelistChannelPolicy extends PlayerListChannelPolicy {
		...
		@Override
		public boolean canPlayerJoin(EntityPlayerMP player) {
			return this.players.contains(player.getCommandSenderName());
		}
