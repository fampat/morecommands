...
public class ClientPlayerSettings {
	...
	public static boolean readSettings(String socketAddress) {
		...
		try {
			...
			if (bindings != null) {
				for (Object o : bindings.getKeySet()) {
			...
			if (aliases != null) {
				for (Object o : aliases.getKeySet()) {
			...
			if (xray != null) {
				for (Object o : xray.getKeySet()) {
					...
					for (Object o2 : colors.getKeySet()) {
			...
			ex.printStackTrace();
			MoreCommands.getMoreCommands().getLogger().warn("Failed to read player settings for player '" + Minecraft.getMinecraft().thePlayer.getName() + "'");
			return false;
		}
	}
	...
	public static boolean saveSettings() {
		try{
			...
			data.setTag(Minecraft.getMinecraft().getNetHandler().getNetworkManager().getRemoteAddress().toString(), socket);
			...
			ex.printStackTrace();
			MoreCommands.getMoreCommands().getLogger().warn("Failed to write player settings for player '" + Minecraft.getMinecraft().thePlayer.getName() + "'");
			return false;
		}
	}
}
