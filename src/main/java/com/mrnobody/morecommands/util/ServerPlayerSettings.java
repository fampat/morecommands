...
public class ServerPlayerSettings {	
	...
	/**
	 * Reads the settings for a player from the disk
	 * 
	...
	public static ServerPlayerSettings readPlayerSettings(EntityPlayerMP player) {
		...
		try {
			...
			if (serverbindings != null) {
				for (Object o : serverbindings.getKeySet()) {
			...
			if (clientbindings != null) {
				for (Object o : clientbindings.getKeySet()) {
			...
			if (serveraliases != null) {
				for (Object o : serveraliases.getKeySet()) {
			...
			if (clientaliases != null) {
				for (Object o : clientaliases.getKeySet()) {
			...
			if (waypoints != null) {
				for (Object o : waypoints.getKeySet()) {
			...
			if (inventories != null) {
				for (Object o : inventories.getKeySet()) {
			...
			ex.printStackTrace();
			MoreCommands.getMoreCommands().getLogger().warn("Failed to read player settings for player '" + player.getName() + "'");
			return settings;
		}
	}
	...
	public boolean saveSettings() {
		try{
			...
			ex.printStackTrace();
			MoreCommands.getMoreCommands().getLogger().warn("Failed to write player settings for player '" + player.getName() + "'");
			return false;
		}
	}
}
