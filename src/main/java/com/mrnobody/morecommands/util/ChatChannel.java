...
public class ChatChannel {
	 ...
	 * @param isChat whether this is a regular chat message
	 */
	public void sendChatMessage(IChatComponent message, boolean isChat) {
		checkState();
		IChatComponent msgToSend = message;
		
		if (MoreCommandsConfig.prefixChannelName)
			msgToSend = new ChatComponentText("[" + this.getName() + "] ").appendSibling(message);
		
		S02PacketChat packet = new S02PacketChat(msgToSend, isChat);
		...
		for (ChatChannel child : this.children)
			if (child.receiveParentMessages)
				child.sendChatMessage(message, isChat);
	}
	...
	private static final class MasterChannel extends ChatChannel {
		...
		public void sendChatMessage(IChatComponent message, boolean isChat) {
			...
			super.sendChatMessage(message, isChat);
		}
