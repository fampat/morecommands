...
public final class GlobalSettings implements TwoEventListener<WorldEvent.Load, WorldEvent.Unload> {
	...
	public void onEvent1(WorldEvent.Load event) {
		...
		loadSetting("aliases", String.class, this.aliases, event.getWorld().getSaveHandler().getWorldDirectory().getName(), event.getWorld().provider.getDimensionType().getName());
		loadSetting("variables", String.class, this.variables, event.getWorld().getSaveHandler().getWorldDirectory().getName(), event.getWorld().provider.getDimensionType().getName());
	}
	...
	public void onEvent2(WorldEvent.Unload event) {
		if (event.getWorld().isRemote) return;
		Pair<String, String> pair = ImmutablePair.of(event.getWorld().getSaveHandler().getWorldDirectory().getName(), event.getWorld().provider.getDimensionType().getName());
	...
	private void registerGlobalAliases(MergedMappedSettings<String, String> aliases) {
		net.minecraft.command.CommandHandler commandHandler = (net.minecraft.command.CommandHandler) FMLCommonHandler.instance().getMinecraftServerInstance().getCommandManager();
