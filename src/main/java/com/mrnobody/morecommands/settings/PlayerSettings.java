...
public abstract class PlayerSettings implements ICapabilityProvider {
	...
	/** The identifier with that an object of this class should be registered to a player */
	...
	public static final void registerCapabilities() {
		CapabilityManager.INSTANCE.register(ClientPlayerSettings.class, new Capability.IStorage<ClientPlayerSettings>() {
			@Override 
			public NBTBase writeNBT(Capability<ClientPlayerSettings> capability, ClientPlayerSettings instance, EnumFacing side) {
				return null;
			}
			
			@Override 
			public void readNBT(Capability<ClientPlayerSettings> capability, ClientPlayerSettings instance, EnumFacing side, NBTBase nbt) {
			
			}
			
		}, new Callable<ClientPlayerSettings>() {
			@Override public ClientPlayerSettings call() throws Exception {
				if (ClientPlayerSettings.getInstance() != null) return ClientPlayerSettings.getInstance();
				else return new ClientPlayerSettings(new DummySettingsManager(true), false);
			}
		});
		
		CapabilityManager.INSTANCE.register(ServerPlayerSettings.class, new Capability.IStorage<ServerPlayerSettings>() {
			@Override 
			public NBTBase writeNBT(Capability<ServerPlayerSettings> capability, ServerPlayerSettings instance, EnumFacing side) {
				return null;
			}
			...
			@Override public ServerPlayerSettings call() throws Exception {
				return new ServerPlayerSettings(new DummySettingsManager(false), false);
			}
		});
	}
	 ...
	 * @see MergedMappedSettings#setSelectedPutSetting(Setting)
	 */
