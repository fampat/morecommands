...
public class CommonPatcher {
	...
	public void clonePlayer(Clone event) {
		...
		this.mod.getPacketDispatcher().sendS14RemoteWorld((EntityPlayerMP) event.getEntityPlayer(), event.getEntityPlayer().world.getSaveHandler().getWorldDirectory().getName());
	}
	...
	public void clientConnect(ServerConnectionFromClientEvent event) {
		...
		this.mod.getPacketDispatcher().sendS14RemoteWorld(player, player.world.getSaveHandler().getWorldDirectory().getName());
